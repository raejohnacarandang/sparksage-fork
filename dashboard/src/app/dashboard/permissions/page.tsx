...
export default function PermissionsPage() {
  ...
  const fetchPermissions = () => {
    if (!token) return;
    fetch("${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/permissions", {
  ...
  const handleAdd = async () => {
    ...
    setMessage(null);
    try {
      const res = await fetch("${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/permissions", {
        method: "POST",
        headers: {
